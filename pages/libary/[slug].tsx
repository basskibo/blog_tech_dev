...
interface TableOfContentItem {
	title: string
	headerType: 'h2' | 'h3'
	subheading: TableOfContentItem[]
	link: string
}

function getTableOfContents(content: string): TableOfContentItem[] {
	const reg = /^(### |## )(.*)\n/
	...
	let lastElType: 'h2' | 'h3' | undefined
	if (headings.length) {
...
interface LibaryDetailsProps {
	data: LibaryData
	mdxSource: MDXRemoteSerializeResult
	toc: TableOfContentItem[]
}

const LibaryDetails: React.FC<LibaryDetailsProps> = ({ data, mdxSource, toc }) => {
	...
	const getViews = async (data: LibaryData) => {
		const url = '/api/views'
...
export const getStaticProps: GetStaticProps<LibaryDetailsProps, { slug: string }> = async (context) => {
	...
	const libaryData: LibaryData = { ...data, slug } as LibaryData
