...
interface DevToError {
	error: string
}

export default async function handler(
	req: NextApiRequest,
	res: NextApiResponse<DevToArticle[] | DevToError>
) {
	if (req.method === 'GET') {
		try {
			...
			const topArticles: DevToArticle[] = sortedArticles.slice(0, 3).map(article => ({
				title: article.title,
