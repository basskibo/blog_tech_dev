...
<Disclosure question={'1. What are the main features of React?'} > 
    <LgList>
        <li>
        <Adjusted>JSX</Adjusted> is a syntax extension to JavaScript which is used with React to describe how UI should look like. The bigest advantage of using JSX is that we can write HTML structures in the same file which contains Javascript code. Html is placed under <code>render()</code> method.
        </li>
        <li>
        <Adjusted>Components</Adjusted> are the building blocks of any React application where we usually use combination of multiple components. It splits the user interface into independent, isolated and reusable parts that can be processed separately and in combination with other components. 
        </li>
        <li>
        <Adjusted>Virtual DOM</Adjusted>: React holds a lightweight copy representation of the real DOM in memory and that represent Virtual DOM. When the state is changed, virtual DOM will compare real with virutal DOM and it will change only object which is changed and not the whole DOM.
        </li>
        <li>
        <Adjusted>One-way data-binding</Adjusted> or <Adjusted>Unidirectional data flow</Adjusted> wher data can be moved in only one way.
        </li>
        <li>
...
<b>JSX</b> is a syntax extension to JavaScript which is used with React to describe how UI should look like. The bigest advantage of using JSX is that we can write HTML structures in the same file which contains Javascript code. Html is placed under <code>render()</code> method.
...
<Disclosure question={'5. What is difference between React and Angular?'}>
        Main differences between these two are: 
<Table header={['','React','Angular']} content={[
['Developer', 'Facebook' , 'Google'],
['Release Year' , '2013', '2009'],
    ['Type', 'Frontend Libary', 'Framework'],
    ['DOM','Virtual DOM', 'Real DOM'],
   ['Data Binding','Uni-directonal (one way binding)', 'Bi-directonal (two way binding)'],
['Language','JSX+JS/TSX+TS', 'TS+HTML'],
['Application Logic','Redux/Flux', 'Services'],
...
<Link href="https://www.jageticbojan.com/post/react-virtualization">Advanced Techniques for Improving Performance with React Virtualization</Link>
</li>
    </LgList>
</Disclosure>

<Disclosure question={'33. Can React Hook replaces Redux?'}>
