---
title: "Push notification with Firebase and React"
...
featuredImage: "https://res.cloudinary.com/dr1sm5gnj/image/upload/v1669639620/igutech/blogs/dominic-sansotta-CePY8WkO9HU-unsplash_v1sdd6.jpg"
...
an account just skip this step and go to the <Link href="https://console.firebase.google.com/"> Firebase console</Link>. Afte we register we need to create new project and when we have a project created we need to register application afterwards which we will get firebase configuration.
...
After you enter name you will be prompted to check or uncheck whether you want to use Google analytic.You can feel free to uncheck because we dont need it know but if you leave it checked it will make no harm. After all data are filled, you will see loader and afterwards will be redirected to the project screen.

### Application registration

When your project is created you will need to choose platform you want to connect to your application, so we are using web project so we can select <b> web </b> option. You will be prompted again to enter name for application you are about to register, after entering name you can procceed and configuration for your application will be generated, which will be needed to get token, connect and use the FCM. This given configuration you can copy and we will provide it to our React application.
...
It will subscribe our application to push notifications (FCM) and it will require permission if its not granted so be sure to <b>Allow notifications</b> in the browser in order to be able to recieve token. If our application have necessary permission we will get token which we wil later use to send push notification.
```
...
4.) Next thing we are required to do before sending push notificaitons, we need to generate <b>VAPID</b> or <b>Voluntary Application Server Identification </b> in order to use <b>getToken</b> method. In order to do this, go to <b>Project Settings</b> on your Firebase console, navigate to the <b>Cloud Messaging</b> tab and go to <b>Web configuration</b> section. You will find <b>Web Push certification</b> tab here you can click on <b>Generate key pair</b>.
...
So we can seperate two ways of receiving messages foreground and background.
...
### Foreground listener
.Foreground listener we alredy added and it will be used when our application is open and focused.
...
FCM requires one specific file which will be called <Adjusted>firebase-messaging-sw.js</Adjusted> inside of <Adjusted>public</Adjusted> folder, so first go ahead and create this file.
Lets explain this part, in order to handle background messages, we will neeed to use <b>service worker</b> which is a script that browser can run in the background completely seperated from the web page and it can enable features that not requires web page or users interaction. 
...
Lets test and checkout wheather it is working as expected, in order to test it go to the Firebase console and send message 
<LgList>
