...
const PostCard = ({ post }) => {
   return (
      <div
         key={post.data.slug}
         className={clsx(post.data.inPreparation ? "text-slate-600 pointer-events-none bg-blend-hard-light bg-neutral-800 " :
          "text-white transform-gpu scale-100 hover:scale-[1.03] active:scale-[0.97] hover:cursor-pointer transition duration-100 animate-shadow",
            "w-full   rounded-md border border-gray-600 dark:bg-dark dark:border-gray-600",
         "")}>
               ...
               <div className='mt-3 text-slate-400 flex-row '>
                  ...
                  <div className='basis-1/2'>
                     <p className='align-middle font-bold text-slate-400 mt-1'>
                        {post.data.inPreparation ? "": moment(post.data.publishedAt).format("MMMM DD, YYYY")}
                     </p>
                  </div>
               </div>

               <p className={clsx(post.data.inPreparation ? "text-slate-500" : "text-slate-400" ,"mt-1  text-sm italic" )}>
                  {post.data.excerpt}
               </p>
            </div>
         </a>
